...
use crate::{graph::{node::PergaminoNode, node_behavior::{GraphContext, NodeAction, NodeCategory, PergaminoNodeBehavior}, types::DataType}, ui::widgets::{node_reference_text_edit::NodeReferenceTextEdit, variable_text_edit::VariableTextEdit}};
...
impl PergaminoNodeBehavior for AnimationNode {
	...
	fn show_body(
		...
		ui.vertical(|ui| {
			...
			ui.vertical(|ui| {
				...
				ui.add_space(8.0);
				let response = VariableTextEdit::new(&mut self.animation, ctx.variables)
					.singleline()
					.desired_width(200.0)
